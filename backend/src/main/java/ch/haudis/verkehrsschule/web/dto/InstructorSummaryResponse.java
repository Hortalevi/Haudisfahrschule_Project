...
public record InstructorSummaryResponse(String id, String name, String username, String color) {
    public static InstructorSummaryResponse from(AppUser user) {
        return new InstructorSummaryResponse(
                user.getId().toString(), user.getName(), user.getUsername(), user.getColor());
    }
}
